...
void keras::LayerDense::load_weights(std::ifstream &fin) {
  ...
  //std::cout << "Input size: " << m_input_cnt << " neurons num: " << m_neurons << std::endl;
  float tmp_float;
  char tmp_char = ' ';
  for(int i = 0; i < m_input_cnt; ++i) {
    ...
    for(int n = 0; n < m_neurons; ++n) {
      fin >> tmp_float;
      //std::cout << "\rFaltam " << m_neurons - n << " neuronios; "
      //<< "Peso: " << tmp_float << std::flush;
...
std::vector<float> keras::KerasModel::compute_output(keras::DataChunk *dc) {
  std::cout << endl << "KerasModel compute output" << std::endl;
  ...
  for(int l = 0; l < (int)m_layers.size(); ++l) {
    std::cout << "Processing layer " << l << " of type: " << m_layers[l]->get_name() << std::endl;
